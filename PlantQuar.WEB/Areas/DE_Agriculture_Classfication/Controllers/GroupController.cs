...
namespace PlantQuar.WEB.Areas.DE_Agriculture_Classfication.Controllers
{
    ...
    {
        // GET: DE_Agriculture_Classfication/Group
        string apiName = "Group_API";

        // GET: MainClassifications/SecondaryClassification
        public ActionResult Index()
        {
            return View();
        }
        [HttpGet]
        public ActionResult AllGroupExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<GroupDTO>>().Result;//object
        ...
        (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "listGroup");
        ...
        //Insert
        [HttpPost]
        public JsonResult CreateGroup(GroupDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreateGroup");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        //UPDATE
        [HttpPost]
        public JsonResult UpdateGroup(GroupDTO model)
        {
            try
            {
                if (ModelState.IsValid)
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdateGroup");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        //DELETE
        [HttpPost]
        public JsonResult DeleteGroup(byte ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteGroup");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
