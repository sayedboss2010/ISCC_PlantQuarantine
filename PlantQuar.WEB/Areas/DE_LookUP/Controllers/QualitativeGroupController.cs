...
namespace PlantQuar.WEB.Areas.DE_LookUP.Controllers
{
    public class QualitativeGroupController : Controller
    {
        string apiName = "QualitativeGroup_API";

        // GET: DE_LookUP/QualitativeGroup
        ...
        //LOAD SEARCH
        [HttpPost]
        public JsonResult listQualitativeGroup
           (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                else
                {
                    ...
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "listShipment_Mean");
        ...
        //Insert
        [HttpPost]
        public JsonResult CreateQualitativeGroup(QualitativeGroupDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreateQualitativeGroupType");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdateQualitativeGroup(QualitativeGroupDTO model)
        {
            try
            {
                if (ModelState.IsValid)
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdateQualitativeGroupType");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //DELETE
        [HttpPost]
        public JsonResult DeleteQualitativeGroup(byte ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteQualitativeGroupType");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        //Eslam_ExportToExcel
        public ActionResult QualitativeGroupExcel()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<QualitativeGroupDTO>>().Result;//object
