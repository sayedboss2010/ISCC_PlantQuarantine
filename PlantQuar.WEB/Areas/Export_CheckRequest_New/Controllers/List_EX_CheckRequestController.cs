...
{
    public class List_EX_CheckRequestController : BaseController
    {
        // GET: EX_CheckRequests/List_ImCheckRequest
        ...
        public ActionResult EX_CheckRequest_List(string DateFrom = "", string DateEnd = "",int selectApproveId=0
            ,int FinalResultListId=0, string CheckRequest_Number="", long Company_ID =0 )
        {
            ...
            try
            {
                ...
                // outlet_ID = 13;
                var userId = (short)Session["UserId"];
                var res = APIHandeling.getData("EX_CheckRequests_API?userId="+ userId + "&DateFrom=" + DateFrom +
        ...
        [HttpPost]
        public JsonResult EX_CheckRequest_List2(string IsApproved = "1", string requestnumber = "")
        {
            try
            {
                var userId = (short)Session["UserId"];
        ...
        public JsonResult CheckRequestStatusLst_AddEDIT()
        {
            try
            {
                ...
                var res = APIHandeling.getData("EX_CheckRequests_API?CheckRequestStatusLst=1");
        ...
        public JsonResult Company_List( long Outlet_ID)
        {
            try
            {
                var res = APIHandeling.getData("Company_National_API?EX_CheckRequest=1&Outlet_ID="+ Outlet_ID);
