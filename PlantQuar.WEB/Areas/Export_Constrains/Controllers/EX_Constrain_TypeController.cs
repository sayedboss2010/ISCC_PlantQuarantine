...
{
    public class EX_Constrain_TypeController : BaseController
    {
        // GET: Export_Constrains/EX_Constrain_Type
        string apiName = "EX_Constrain_Type_API";
        ...
        [HttpGet]
        public ActionResult AllEX_Constrain_TypeExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<EX_Constrain_TypeDTO>>().Result;//object
        ...
        //LOAD SEARCH
        [HttpPost]
        public JsonResult listEX_Constrain_Type
        ...
        [HttpPost]
        public JsonResult CreateEX_Constrain_Type(EX_Constrain_TypeDTO model)
        {
            try
            {
        ...
        [HttpPost]
        public JsonResult UpdateEX_Constrain_Type(EX_Constrain_TypeDTO model)
        {
            try
            {
                if (ModelState.IsValid)
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdateEX_Constrain_Type");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        [HttpPost]
        public JsonResult DeleteEX_Constrain_Type(byte ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteEX_Constrain_Type");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //CHECK CODE
        [HttpPost]
        public JsonResult EX_Constrain_Type_List()
        {
            try
            {
            ...
            {
                APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "EX_Constrain_Type_List");
                return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
            }
        }

        [HttpPost]
        public JsonResult EX_Constrain_Type_AddEDIT()
        {
            try
            {
