...
{
    public class ShippingAgencyController : BaseController
    {
        // GET: Shipping/ShippingAgency
        string apiName = "ShippingAgency_API";
        public ActionResult Index()
        {
            return View();
        }
        [HttpGet]
        public ActionResult AllShippingAgencyExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<ShippingAgencyDTO>>().Result;//object
        ...
        //LOAD SEARCH
        [HttpPost]
        public JsonResult listShippingAgencys
        (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                else
                {
                    ...
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "listShippingAgencys");
        ...
        //Insert
        [HttpPost]
        public JsonResult CreateShippingAgencys(ShippingAgencyDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreateShippingAgency");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdateShippingAgencys(ShippingAgencyDTO model)
        {
            try
            {
                if (ModelState.IsValid)
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdateShippingAgency");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //DELETE
        [HttpPost]
        public JsonResult DeleteShippingAgencys(Int16 ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteShippingAgencys");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        //check code
        [HttpPost]
        public JsonResult GetShippingAgencyOptions()
        {
            try
            {
                var res = APIHandeling.getData(apiName + "?IsActive=1");
                var lst = res.Content.ReadAsAsync<List<ShippingAgencyDTO>>().Result;
                var ShippingAgencys = lst.Select(c => new { DisplayText = c.Name_Ar, Value = c.ID, }).OrderBy(s => s.DisplayText);

                return Json(new { Result = "OK", Options = ShippingAgencys });
            }
            catch (Exception ex)
            {
