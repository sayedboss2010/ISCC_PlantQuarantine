...
namespace PlantQuar.WEB.Areas.Shipping.Controllers
{
    ...
    {
        // GET: Shipping/ShippingCompany
        string apiName = "ShippingCompany_API";
        public ActionResult Index()
        {
            return View();
        }
        [HttpGet]
        public ActionResult AllShippingCompanyExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<ShippingCompanyDTO>>().Result;//object
                        return Json(lst, JsonRequestBehavior.AllowGet);
        }
        //LOAD SEARCH
        [HttpPost]
        public JsonResult listShippingCompanys
        (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                else
                {
                    ...
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "listShippingCompanys");
        ...
        //Insert
        [HttpPost]
        public JsonResult CreateShippingCompanys(ShippingCompanyDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreateShippingCompany");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdateShippingCompanys(ShippingCompanyDTO model)
        {
            try
            {
                if (ModelState.IsValid)
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdateShippingCompany");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //DELETE
        [HttpPost]
        public JsonResult DeleteShippingCompanys(Int16 ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteShippingCompanys");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        //check code
        [HttpPost]
        public JsonResult GetShippingCompanyOptions()
        {
            try
            {
                var res = APIHandeling.getData(apiName + "?IsActive=1");
                var lst = res.Content.ReadAsAsync<List<ShippingCompanyDTO>>().Result;
                var ShippingCompanys = lst.Select(c => new { DisplayText = c.Name_Ar, Value = c.ID, }).OrderBy(s => s.DisplayText);

                return Json(new { Result = "OK", Options = ShippingCompanys });
            }
            catch (Exception ex)
            {
