...
namespace PlantQuar.WEB.Areas.DE_Packages.Controllers
{
    public class PackageMaterialController : BaseController
    {
        // GET: DE_Packages/PackageMaterial
        string apiName = "PackageMaterial_API";
        ...
        [HttpGet]
        public ActionResult AllPackageMaterial()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<PackageMaterialDTO>>().Result;//object
        ...
        [HttpPost]
        public JsonResult listPackage_Material
            (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "listPackage_Material");
        ...
        [HttpPost]
        public JsonResult CreatePackage_Material(PackageMaterialDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreatePackage_Material");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        [HttpPost]
        public JsonResult UpdatePackage_Material(PackageMaterialDTO model)
        {
            try
            {
                if (ModelState.IsValid)
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdatePackage_Material");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        [HttpPost]
        public JsonResult DeletePackage_Material(short ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeletePackage_Material");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
