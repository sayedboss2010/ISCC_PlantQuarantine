...
namespace PlantQuar.WEB.Areas.DE_Packages.Controllers
{
    ...
    {
        // GET: DE_Packages/PackageType
        string apiName = "PackageType_API";
        ...
        [HttpGet]
        public ActionResult AllPackageType()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<PackageTypeDTO>>().Result;//object
        ...
        //LOAD SEARCH
        [HttpPost]
        public JsonResult listPackage_Type
           (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "listPackage_Type");
        ...
        //NEW
        [HttpPost]
        public JsonResult CreatePackage_Type(PackageTypeDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreatePackage_Type");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdatePackage_Type(PackageTypeDTO model)
        {
            try
            {
                //var allErrors = ModelState.Values.SelectMany(x => x.Errors).ToList();
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdatePackage_Type");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //DELETE
        [HttpPost]
        public JsonResult DeletePackage_Type(int ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeletePackage_Type");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
