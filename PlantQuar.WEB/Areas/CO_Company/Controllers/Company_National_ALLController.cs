...
{
    public class Company_National_ALLController : BaseController
    {
        // GET: CO_Company/Company_National_ALL
        //CHECK ALL CODE
        string apiName = "Company_National_API";

        public ActionResult Index()
        {
            var model = new CompanyNationalDTO();
            var res = APIHandeling.getData("ContactType_API?List=1");
            var lst = res.Content.ReadAsAsync<List<CustomOption>>().Result;
            ViewBag.ContactTypelst = lst;
            return View(model);
        }

        //LOAD SEARCH
        [HttpPost]
        public JsonResult listCompanyNational
        (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "listCompanyNational");
        ...
        //Insert
        [HttpPost]
        public JsonResult CreateCompanyNational(CompanyNationalDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreateCompanyNational");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        [HttpPost]
        public JsonResult UpdateCompanyNational(CompanyNationalDTO model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteCompanyNational(long ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteCompanyNational");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
