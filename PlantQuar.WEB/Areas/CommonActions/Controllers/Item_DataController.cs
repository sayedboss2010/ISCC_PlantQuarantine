...
{
    public class Item_DataController : Controller
    {
        // GET: CommonActions/Item_Data
        #region Item_Data

        [HttpPost]
        public JsonResult Item_Data_List()
        {
            try
            {
        ...
        [HttpPost]
        public JsonResult Item_Data_AddEDIT()
        {
            try
            {
                var res = APIHandeling.getData("MainClassification_API?AddEdit=1");
                var lst = res.Content.ReadAsAsync<List<CustomOption>>().Result;
                return Json(new { Result = "OK", Options = lst });
            }
            catch (Exception ex)
