...
namespace PlantQuar.WEB.Areas.DE_Scientific_Classfication.Controllers
{
    ...
    {
        // GET: DE_Scientific_Classfication/Level
        string apiName = "Level_API";
        public ActionResult Index()
        {
            return View();
        }
        public ActionResult AllLevelExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<LevelDTO>>().Result;//object
        ...
        //Insert
        [HttpPost]
        public JsonResult CreateLevel(LevelDTO model)
        {
            try
            {
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdateLevel(LevelDTO model)
        {
            try
            {
                if (ModelState.IsValid)
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdateLevel");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //DELETE
        [HttpPost]
        public JsonResult DeleteLevel(byte ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteLevel");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
