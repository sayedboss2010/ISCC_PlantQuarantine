...
namespace PlantQuar.WEB.Areas.DE_Scientific_Classfication.Controllers
{
    ...
    {
        // GET: DE_Scientific_Classfication/Order
        string apiName = "Order_API";
        ...
        (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtPageSize = 0, int jtStartIndex = 0, string jtSorting = "")
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                else
                {
                    ...
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "listOrder");
        ...
        //NEW
        [HttpPost]
        public JsonResult CreateOrders(OrderDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreateOrder");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdateOrders(OrderDTO model)
        {
            try
            {
                //var allErrors = ModelState.Values.SelectMany(x => x.Errors).ToList();
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdateOrder");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        public JsonResult DeleteOrder(int ID)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteOrder");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        public ActionResult AllOrderExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");
            var lst = res.Content.ReadAsAsync<List<OrderDTO>>().Result;//object
