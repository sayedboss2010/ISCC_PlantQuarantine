...
{
    public class PortTypeController : BaseController
    {
        // GET: DE_Port/PortType
        string apiName = "PortType_API";
        ...
        [HttpGet]
        public ActionResult AllPortTypeExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<Port_TypeDTO>>().Result;//object
        ...
        //LOAD SEARCH
        public JsonResult listPortType
           (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    //entity.Sp_plant_Error_Insert(Request.Url.AbsoluteUri.ToString(), ex.Message, "listPortTypes");
        ...
        //NEW
        [HttpPost]
        public JsonResult CreatePortType(Port_TypeDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    // APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreatePortTypes");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdatePortType(Port_TypeDTO model)
        {
            try
            {
                //var allErrors = ModelState.Values.SelectMany(x => x.Errors).ToList();
            ...
            catch (Exception ex)
            {
                ...
                {
                    // APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdatePortTypes");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //DELETE
        [HttpPost]
        public JsonResult DeletePortType(byte Id)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeletePortTypes");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
