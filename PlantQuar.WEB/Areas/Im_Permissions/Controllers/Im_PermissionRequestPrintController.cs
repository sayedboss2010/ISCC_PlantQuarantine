...
namespace PlantQuar.WEB.Areas.Im_Permissions.Controllers
{
    ...
    {
        // GET: Im_Permissions/Im_PermissionRequestPrint
        public ActionResult Index(decimal? ImPermission_Number)
        {
            User_Session Current = User_Session.GetInstance;
            var us = (short)Session["UserId"];
            var name = Session["FullName"];
            var canprint =(bool) Session["CanPrint"];
            var CanView = Session["CanView"];
            var User_Creation_Id = (short)Session["UserId"];

            var res = APIHandeling.getData("ListIm_PermissionAPI?print=1&ImPermission_Number=" + ImPermission_Number+ "&User_Creation_Id="+ User_Creation_Id);
        ...
        public ActionResult PrintArabic(long Im_PermissionRequest_ID)
        {
            ...
            var res = APIHandeling.Put("ListIm_PermissionAPI?printAr=1", printArabic);
            return Json("success");
        }
        [HttpPost]
        public ActionResult PrintEnglish(long Im_PermissionRequest_ID)
        {
            ...
            var res = APIHandeling.Put("ListIm_PermissionAPI?printEn=1", printEnglish);
            return Json("success");
        }
