...
namespace PlantQuar.WEB.Areas.DE_Item_Descriptions.Controllers
{
    public class Items_StrainController : BaseController
    {
        // GET: DE_Item_Descriptions/Items_Strain
        string apiName = "ItemCategories_TypeAPI";
        ...
        //LOAD SEARCH
        public JsonResult listItems_Strains
           (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "listItems_Strain");
        ...
        //NEW
        [HttpPost]
        public JsonResult CreateItems_Strains(ItemCategories_TypeDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreateItems_Strain");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdateItems_Strains(ItemCategories_TypeDTO model)
        {
            try
            {
                //var allErrors = ModelState.Values.SelectMany(x => x.Errors).ToList();
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdateItems_Strain");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //DELETE
        [HttpPost]
        public JsonResult DeleteItems_Strains(Int16 id)
        {
            try
            {
                DeleteParameters deleteParameters = new DeleteParameters();
                deleteParameters.id = id;
                deleteParameters._DateNow = DateTime.Now;
                User_Session Current = User_Session.GetInstance;
                deleteParameters.Userid = (short)Session["UserId"];
                APIHandeling.Delete(apiName, deleteParameters);
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteItems_Strain");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        //Eslam_ExportToExcel
        public ActionResult Items_StrainExcel()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<ItemCategories_TypeDTO>>().Result;//object
