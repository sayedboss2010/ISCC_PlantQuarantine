...
{
    public class SubPartController : BaseController
    {
        // GET: DE_Item_Descriptions/SubPart
        string apiName = "SubPart_API";
        ...
        //Insert
        [HttpPost]
        public JsonResult CreatePlantPartType(SubPartDTO model)
        {
            try
            {
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdatePlantPartType(SubPartDTO model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        public ActionResult GetSubPartListExcel()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<SubPartDTO>>().Result;//object
