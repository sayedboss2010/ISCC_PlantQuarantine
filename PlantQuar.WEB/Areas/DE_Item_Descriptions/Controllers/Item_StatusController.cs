...
namespace PlantQuar.WEB.Areas.DE_Item_Descriptions.Controllers
{
    ...
    {
        // GET: DE_Item_Descriptions/Item_Status
        string apiName = "Item_Status_API";
        ...
        //Insert
        [HttpPost]
        public JsonResult CreateItem_Status(Item_StatusDTO model)
        {
            try
            {
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdateItem_Status(Item_StatusDTO model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        //Eslam_ExportToExcel
        public ActionResult Item_StatusExcel()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<Item_StatusDTO>>().Result;//object
