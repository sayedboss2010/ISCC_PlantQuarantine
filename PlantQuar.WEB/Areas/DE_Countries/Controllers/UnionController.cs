...
namespace PlantQuar.WEB.Areas.DE_Countries.Controllers
{
    ...
    {
        // GET: DE_Countries/Union
        string apiName = "Union_API";
        public ActionResult Index()
        {
            return View();
        }
        [HttpGet]
        public ActionResult AllUnionExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<UnionDTO>>().Result;//object
        ...
        (string txt_AR_BTNSearch = "", string txt_EN_BTNSearch = "", int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                else
                {
                    ...
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "listUnions");
        ...
        //Insert
        [HttpPost]
        public JsonResult CreateUnions(UnionDTO model)
        {
            try
            {
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdateUnions(UnionDTO model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        //DELETE
        [HttpPost]
        public JsonResult DeleteUnions(Int16 ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteUnions");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        //check code
        [HttpPost]
        public JsonResult GetUnionOptions()
        {
            try
            {
                var res = APIHandeling.getData(apiName + "?IsActive=1");
                var lst = res.Content.ReadAsAsync<List<UnionDTO>>().Result;
                var Unions = lst.Select(c => new { DisplayText = c.Ar_Name, Value = c.ID, }).OrderBy(s => s.DisplayText);

                return Json(new { Result = "OK", Options = Unions });
            }
            catch (Exception ex)
            {
