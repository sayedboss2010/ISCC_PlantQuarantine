...
{
    public class RegionAreaController : BaseController
    {
        // GET: DE_Countries/RegionArea
        string apiName = "RegionArea_API";
        ...
        [HttpGet]
        public ActionResult AllRegionAreaExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<Regional_AreaDTO>>().Result;//object
        ...
        //NEW
        [HttpPost]
        public JsonResult CreateRegion(Regional_AreaDTO model)
        {
            try
            {
        ...
        //UPDATE
        [HttpPost]
        public JsonResult UpdateRegion(Regional_AreaDTO model)
        {
            try
            {
                //var allErrors = ModelState.Values.SelectMany(x => x.Errors).ToList();
