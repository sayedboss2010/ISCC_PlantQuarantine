...
{
    public class CommitteeTypeController : BaseController
    {
        string apiName = "CommitteeType";

        // GET: Committees/CommitteeType
        ...
        [HttpPost]
        public JsonResult CreateCommitteeType(CommitteeTypeDTO model)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "CreateCommitteeType");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        [HttpPost]
        public JsonResult UpdateCommitteeType(CommitteeTypeDTO model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteCommitteeType(byte ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteCommitteeType");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        [HttpGet]
        public ActionResult AllCommitteeTypeExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<CommitteeTypeDTO>>().Result;//object
