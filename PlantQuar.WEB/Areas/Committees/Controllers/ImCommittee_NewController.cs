...
{
    public class ImCommittee_NewController : BaseController
    {
        //Eslam
        ...
        public ActionResult Index(long? requestId, long? OutLet_ID)
        {
            ...
            var resItem = APIHandeling.getData("Im_CheckRequests_New_API?ImCheckRequest_Number=" + req.CheckRequest_Number);
    ...
    DateTime? Check_Date, DateTime? Delegation_Date, TimeSpan? StartTime, TimeSpan? EndTime)
            ...
            , List<Im_CheckRequest_SampleDataDTO> CheckedAnalysisList)
        {
            try
            {
                ...
                var CheckedAnalysisList_New = new List<Im_CheckRequest_SampleDataDTO>();
                //if (ModelState.IsValid)
                //{
                if (CheckedItemsList != null && CheckedItemsList.Count != 0)
                {
                        ...
                        if (CheckedAnalysisList != null && CheckedAnalysisList.Count != 0)
                        {
                            foreach (var item_Analysis in CheckedAnalysisList)
                            {
                               long Short_ID = item.Item_ShortName_ID;
                                ...
                                {
                                    var location = new Im_CheckRequest_SampleDataDTO
                                    {
                                    ...
                                    //item_Analysis.Text_Lot = item.Text_Lot;
                    ...
                    if (CommitteeType_ID == "11")
                    {
                        ...
                        var res2 = APIHandeling.Post("ImRequestCommittee_API" + "?AnalysisList=1", CheckedAnalysisList_New);
                    ...
                    else if (CommitteeType_ID == "13")
                    {
                        ...
                        var res2 = APIHandeling.Post("ImRequestCommittee_API" + "?AnalysisList=1", CheckedAnalysisList_New);
                        return ((int)res2.StatusCode != 409) ? Json(new { Result = "OK", Record = CheckedAnalysisList_New })
                              : Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.RepeatedData) });
                    }
                    else
                    {
