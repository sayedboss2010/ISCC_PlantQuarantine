...
{
    public class Refuse_ReasonController : Controller
    {
        // GET: ExportRequest/Refuse_Reason

        string apiName = "Refuse_Reasons_API";
        public ActionResult Index()
        {
            return View();
        }
        [HttpGet]
        public ActionResult AllRefuse_Reasons()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<EX_Refuse_ReasonsDTO>>().Result;//object
        ...
        //LOAD SEARCH 
        [HttpPost]
        public JsonResult listRefuse_Reason
        ...
        //Insert into Refuse_Reason
        [HttpPost]
        public JsonResult CreateRefuse_Reason(EX_Refuse_ReasonsDTO model)
        {
            try
            {
        ...
        //Update  AnlaysisLab
        [HttpPost]
        public JsonResult UpdateRefuse_Reason(EX_Refuse_ReasonsDTO model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteRefuse_Reason(int ID)
        {
            try
            {
        ...
        #region A_SystemCode
        [HttpPost]
        public JsonResult A_SystemCode_List()
        {
            try
            {
                var res = APIHandeling.getData("A_SystemCode_API?List=1");
                var lst = res.Content.ReadAsAsync<List<CustomOption>>().Result;
                return Json(new { Result = "OK", Options = lst });
            }
            catch (Exception ex)
            {
                APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "A_SystemCode_List");
                return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
            }
        }
