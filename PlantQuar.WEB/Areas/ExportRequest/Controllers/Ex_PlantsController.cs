...
{
    public class Ex_PlantsController : BaseController
    {
        // GET: ExportRequest/Ex_Plants
        public ActionResult Ex_Plants()
        {
            return View();
        }
        public ActionResult Ex_Plants2()
        {
            return View();
        }
        //********PLANT DATA
        ...
        public JsonResult Plant_List()
        {
            try
            {
                var res = APIHandeling.getData("Plant_API?plant=1");
        ...
        public JsonResult PlantPartsList(int plantId = 0)
        {
            try
            {
                var res = APIHandeling.getData("PlantPart_API?allowed=true&plantid=" + plantId);
                var lst = res.Content.ReadAsAsync<List<CustomOptionLongId>>().Result;
                return Json(new { Result = "OK", Options = lst });
            }
            catch (Exception ex)
        ...
        public JsonResult GetPlantShortName(long plantId = 0, byte purposeId = 0, byte statusId = 0, int partType = 0)
        {
            try
            {
        ...
        public JsonResult ProductStatus_AddEDIT()
        {
            try
            {
                var res = APIHandeling.getData("PlantPart_API?AddEdit=1");
            ...
            (int plantId = 0, int purposeId = 0, int statusId = 0, int partType = 0, int importerCountryId = 0, int transitCountryId = 0)
        {
            var res = APIHandeling.getData
                ("Export_CheckRequest_API?plantId=" + plantId + "&purposeId=" + purposeId + "&statusId=" + statusId + "&partType=" + partType + "&importerCountryId=" + importerCountryId + "&transitCountryId=" + transitCountryId);
        ...
        //eman
        public JsonResult ListPlantsRows2(long checkReqId, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            var res = APIHandeling.getData("Ex_Items_API" + "?plant=4&requestId=" + checkReqId);
            var modelLst = res.Content.ReadAsAsync<List<Ex_plants>>().Result;
            //var plantsList = new List<Custom_ExPlants>();
            return Json(new { Result = "OK", Records = modelLst, TotalRecordCount = modelLst.Count });
        }
        ...
        public JsonResult ListPlants_LotRows2(long PlantId, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            var res = APIHandeling.getData("Ex_ItemsLots_API" + "?ItemId=" + PlantId);
            var modelLst = res.Content.ReadAsAsync<List<Item_GetLotData_ResultDTO>>().Result;
        ...
        public JsonResult GetGrossWeight(int count, decimal ton, decimal kilo, decimal gram)
        {
            var packageWight = WeightCalculation.ConvertWeightToKilo(ton, kilo, gram);
            var grossWeght = packageWight * count;
            var tonGrossWeight = WeightCalculation.ReCalculate(grossWeght);

            return Json(new { ton = tonGrossWeight[0].ToString(), kilo = tonGrossWeight[1].ToString(), gram = tonGrossWeight[2].ToString() }, JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetPackageNetWeight(int count, decimal ton, decimal kilo, decimal gram)
        {
            var netWight = WeightCalculation.ConvertWeightToKilo(ton, kilo, gram);
            var packageNetWeght = netWight / count;
            var tonGrossWeight = WeightCalculation.ReCalculate(packageNetWeght);
