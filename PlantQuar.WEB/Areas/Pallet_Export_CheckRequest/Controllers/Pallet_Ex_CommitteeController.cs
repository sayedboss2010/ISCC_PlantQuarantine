...
{
    public class Pallet_Ex_CommitteeController : BaseController
    {
        // GET: Pallet_Export_CheckRequest/Pallet_Ex_Committee
        string api = "Pallet_Ex_Committee_API";
        public ActionResult Index(long? requestId, long? OutLet_ID)
        {
            var res = APIHandeling.getData("Pallet_Ex_Committee_API?requestId=" + requestId);
            var req = res.Content.ReadAsAsync<Pallet_EX_CheckRequest_Committee_DTO>().Result;
            ...
            var return_Status = res_Status.Content.ReadAsAsync<List<Pallet_Get_Ex_CountryConstrain_DTO>>().Result;
            ...
            var Lst = resItem.Content.ReadAsAsync<Pallet_Ex_Committee_DTO>().Result;
            Session["listshiftTimingsession"] = new List<Pallet_EX_RequestCommittee_ShiftDTO>();
      ...
      (long EX_RequestCommittee_ID = 0, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            try
            {
                var companiescount = Session["listshiftTimingsession"] as List<Pallet_EX_RequestCommittee_ShiftDTO>;
        ...
        [HttpPost]
        public JsonResult CreateshiftTiming(Pallet_EX_RequestCommittee_ShiftDTO model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var companies = Session["listshiftTimingsession"] as List<Pallet_EX_RequestCommittee_ShiftDTO>;
        ...
        public JsonResult Save_Committe(byte? CommitteeType_ID, long? CheckRequest_Id,
         DateTime? Check_Date, DateTime? Delegation_Date, TimeSpan? StartTime, TimeSpan? EndTime, List<Pallet_EX_EmployeeDTO> emp_Dto,
         List<Pallet_EX_CommitteeResultDTO> CheckedItemsList
           , List<Pallet_EX_CheckRequest_SampleDataDTO> CheckedAnalysisList
            , List<Pallet_EX_Checked_TreatmentMethodDTO> Checked_TreatmentMethod)
        {
            if (CheckedItemsList != null)
            {
                ...
                string barcode = "73" + rand + dayofyear.Substring(dayofyear.Length - 3) + zx + hour + min + sec;
                //   ViewBag.StationCode = barcode;

                #region اللجنة
                Pallet_EX_RequestCommitteeDTO comm = new Pallet_EX_RequestCommitteeDTO();
                ...
                // النوبتجية
                List<Pallet_EX_RequestCommittee_ShiftDTO> Shift_Data = new List<Pallet_EX_RequestCommittee_ShiftDTO>();
                Shift_Data = Session["listshiftTimingsession"] as List<Pallet_EX_RequestCommittee_ShiftDTO>;
                ...
                var CheckedAnalysisList_New = new List<Pallet_EX_CheckRequest_SampleDataDTO>();
                var Checked_TreatmentMethod_New = new List<Pallet_EX_Checked_TreatmentMethodDTO>();
                        ...
                        if (CheckedAnalysisList != null && CheckedAnalysisList.Count != 0)
                        {
                            foreach (var item_Analysis in CheckedAnalysisList)
                            {
                                ...
                                if (item_Analysis.EX_Request_Item_Id == Short_ID)
                                {
                                    ...
                                    var location = new Pallet_EX_CheckRequest_SampleDataDTO
                                    {
                        ...
                        if (Checked_TreatmentMethod != null && Checked_TreatmentMethod.Count != 0)
                        {
                            foreach (var item_Treatment in Checked_TreatmentMethod)
                            {
                                ...
                                if (item_Treatment.EX_Request_Item_Id == Short_ID)
                                {
                                    //if (l)
                                    //var asd = Committee_Sample_Lot
                                    var location = new Pallet_EX_Checked_TreatmentMethodDTO
                                    {
        ...
        [HttpPost]
        public JsonResult GetAllTreatmentDataForShortnameId(long EX_CheckRequest_ID, long ExportCountry_Id, long shortnameId)

        {
            try
            {
                var res = APIHandeling.getData(api + "?EX_CheckRequest_ID=" + EX_CheckRequest_ID + "&ExportCountry_Id=" + ExportCountry_Id + "&shortnameId=" + shortnameId);
                var lst = res.Content.ReadAsAsync<Pallet_AllTreatmentDataForShortnameIdDTO>().Result;
                return Json(new { Result = "OK", Options = lst });
            }
            catch (Exception ex)
            {
