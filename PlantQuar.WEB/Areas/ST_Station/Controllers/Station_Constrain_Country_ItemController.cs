...
{
    public class Station_Constrain_Country_ItemController : BaseController
    {
        // GET: ST_Station/Station_Constrain_Country_Item
        string apiName = "Station_Constrain_Country_Item_API";
        ...
        [HttpGet]
        public ActionResult AllStation_Constrain_Country_ItemExcell()
        {
            var res = APIHandeling.getData(apiName + "?pageSize=-1&index=-1");

            var lst = res.Content.ReadAsAsync<List<Station_Constrain_Country_ItemDTO>>().Result;//object
        ...
        //LOAD SEARCH
        [HttpPost]
        public JsonResult listStation_Constrain_Country_Item
        ...
        [HttpPost]
        public JsonResult CreateStation_Constrain_Country_Item(Station_Constrain_Country_ItemDTO model)
        {
            try
            {
        ...
        [HttpPost]
        public JsonResult UpdateStation_Constrain_Country_Item(Station_Constrain_Country_ItemDTO model)
        {
            try
            {
                if (ModelState.IsValid)
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "UpdateStation_Constrain_Country_Item");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        [HttpPost]
        public JsonResult DeleteStation_Constrain_Country_Item(byte ID)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "DeleteStation_Constrain_Country_Item");
                    return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
                }
            }
        }
        ...
        //CHECK CODE
        [HttpPost]
        public JsonResult Station_Constrain_Country_Item_List( int List)
        {
            try
            {
            ...
            {
                APIHandeling.Insert_Error(Request.Url.AbsoluteUri.ToString(), ex.Message, "Station_Constrain_Country_Item_List");
                return Json(new { Result = "ERROR", Message = Enums.GetEnumDescription<Enums.Error>(Enums.Error.ErrorHappened) });
            }
        }

        [HttpPost]
        public JsonResult Station_Constrain_Country_Item_AddEDIT()
        {
            try
            {
