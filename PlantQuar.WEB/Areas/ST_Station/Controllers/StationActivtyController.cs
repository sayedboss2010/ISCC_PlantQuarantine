...
namespace PlantQuar.WEB.Areas.ST_Station.Controllers
{
    ...
    {
        // GET: ST_Station/StationActivty
        string apiName = "StationActivty_API";
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if ((int)data.StatusCode != 409)
                    {
                        ...
                        {
                            APIHandeling.Post("StationCompany_API?user_id=" + user_id + "&Date_Now=" + Date_Now.ToString("yyyy-MM-dd hh:mm:ss") + "&StationActivityID=" + StationActivityID, CompanyDto);
                        }
                        //country api
                        ...
                        {
                            var country_data = APIHandeling.Post("StationActivityCountry_API?user_id=" + user_id + "&Date_Now=" + Date_Now.ToString("yyyy-MM-dd hh:mm:ss") + "&StationActivityID=" + StationActivityID, CountryDto);
                        }
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if ((int)data.StatusCode != 409)
                    {
                        ...
                        {
                            var comp_data = APIHandeling.Put("StationCompany_API?user_id=" + user_id + "&Date_Now=" + Date_Now.ToString("yyyy-MM-dd hh:mm:ss") + "&StationID=" + model.ID, model.Company);
                        }
                        //update country station api
                        model.CountryID.Remove(0);
                        if (model.CountryID.Count != 0)
                        {
                            APIHandeling.Put("StationActivityCountry_API?user_id=" + user_id + "&Date_Now=" + Date_Now.ToString("yyyy-MM-dd hh:mm:ss") + "&StaionActivityID=" + model.ID, model.CountryID);
                        }
        ...
        public JsonResult GetCompanyListByStationActivityID(int StationActivityID = 0)
        {
            try
            {
                ...
                var res = APIHandeling.getData("StationCompany_API?StationActivityID=" + StationActivityID);
