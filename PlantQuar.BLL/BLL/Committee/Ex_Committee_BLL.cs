...
namespace PlantQuar.BLL.BLL.Committee
{
    public class Ex_Committee_BLL
    {
            ...
            try
            {
                ...
                using (PlantQuarantineEntities context = new PlantQuarantineEntities())
                {
                    using (System.Data.Entity.DbContextTransaction trans = context.Database.BeginTransaction())
                    {
                        ...
                        if (entity.CommitteeType_ID == 2 || entity.CommitteeType_ID == 1 || entity.CommitteeType_ID == 3)//فى حالة الجشني
                        { _IsApproved = true; }
                        ...
                        RequestCommittee.ExCheckRequest_ID = entity.EX_CheckRequest_ID;
                        RequestCommittee.CommitteeType_ID = entity.CommitteeType_ID;
                        RequestCommittee.ExCommitteeCheckLocation_ID = entity.EX_CommitteeCheckLocation_ID;
                        RequestCommittee.Delegation_Date = entity.Delegation_Date;
                        RequestCommittee.StartTime = entity.StartTime;
                        RequestCommittee.EndTime = entity.EndTime;
                        RequestCommittee.IsFinishedAll = entity.IsFinishedAll;
                        //make approval is true if  geshny //(lang == "1" ? c.Name_Ar : c.Name_Ar),
                        RequestCommittee.IsApproved = _IsApproved;//?true:null,
                        RequestCommittee.Status = entity.Status;
                        RequestCommittee.User_Creation_Id = entity.User_Creation_Id;
                        RequestCommittee.User_Creation_Date = entity.User_Creation_Date;
                        if (entity.List_Committee_Shift == null && entity.List_Ex_Request_Fees_Eng == null && entity.CommitteeType_ID != 3)
                        {
                        ...
                        //        var Cmodel = uow.Repository<Ex_RequestCommittee>().Findobject(RequestCommittee.ID);
                        //        Cmodel.IsPaid = true;
                        //        context.SaveChanges();
                        ...
                        if (entity.List_CommitteeResult != null && entity.List_CommitteeResult.Count > 0)
                        {
                            foreach (var item in entity.List_CommitteeResult)
                            {
                                ...
                                var Check_Gashny_ID = context.Ex_CommitteeResult
                                  .Where(a => a.LotData_ID == item.LotData_ID && a.CommitteeResultType_ID == item.CommitteeResultType_ID).ToList();
                                if (Check_Check_ID.Count() == 0)
                                {
                                    Countnu_GashnyAndCheck = 1;
                                }
                                else
                                {
                                    if (Check_Check_ID.LastOrDefault().CommitteeResultType_ID == null)
                                    {
                                        if (Check_Check_ID.LastOrDefault().Ex_RequestCommittee.IsApproved == true)
                                        {
                                            if (Check_Check_ID.LastOrDefault().Ex_RequestCommittee.Delegation_Date < DateTime.Now.Date) // يقوم بتشكيل اللجنة
                                            {
                                                Countnu_GashnyAndCheck = 1;
                                            }
                                            else
                                            {
                                                Countnu_GashnyAndCheck = 0;
                                            }
                                        }
                                        else
                                        {
                                ...
                                if (Check_Gashny_ID.Count() == 0)
                                {
                                    Countnu_GashnyAndCheck = 1;
                                }
                                else
                                {
                                    if (Check_Gashny_ID.LastOrDefault().CommitteeResultType_ID == null)
                                    {
                                        if (Check_Gashny_ID.LastOrDefault().Ex_RequestCommittee.IsApproved == true)
                                        {
                                            if (Check_Gashny_ID.LastOrDefault().Ex_RequestCommittee.Delegation_Date < DateTime.Now.Date) // يقوم بتشكيل اللجنة
                                            {
                                                Countnu_GashnyAndCheck = 1;
                                            }
                                            else
                                            {
                                                if (Check_Gashny_ID.LastOrDefault().Ex_RequestCommittee.Is_Cancel == 1)
                                                {
                                                    Countnu_GashnyAndCheck = 1;
                                                }
                                                else
                                ...
                                if (item.CommitteeResultType_ID == 1)
                                {
                                    var Check_ID = context.Ex_CheckRequest_Items_Lot_Result
                                       .Where(a => a.Ex_CheckRequest_Items_Lot_Category_ID == item.LotData_ID
                                 && (a.IS_Status == 7)).FirstOrDefault();
                                    if (Check_ID != null)
                                    {

                                        if (Check_ID.IS_Status == 7)
                                        {
                                            Countnu_GashnyAndCheck = 1;
                                        }
                                    }
                                }
                                else if (item.CommitteeResultType_ID == 2)
                                {
                                    var Check_ID = context.Ex_CheckRequest_Items_Lot_Result
                                     .Where(a => a.Ex_CheckRequest_Items_Lot_Category_ID == item.LotData_ID
                               && (a.IS_Status == 10)).FirstOrDefault();
                                    if (Check_ID != null)
                                    {

                                        if (Check_ID.IS_Status == 10)
                                        {
                                            Countnu_GashnyAndCheck = 1;
                                        }
                                    }
        ...
        public Dictionary<string, object> Get_Fees(long CheckRequest_ID_Ex, List<string> Device_Info)
        {
            try
            {
                PlantQuarantineEntities entities = new PlantQuarantineEntities();
                ...
                var Fees_Dat = (from ch in entities.Ex_CheckRequest
                                where ch.ID == CheckRequest_ID_Ex
                                select new Fees_ALL
                                {
                                    ID = ch.ID,
                ...
                var item_Fees = (from im_i in entities.Ex_CheckRequest_Items
                                 ...
                                 where im_i.Ex_CheckRequest_ID == CheckRequest_ID_Ex
                                 group im_i by new
                                 {
                ...
                item_Fees.FirstOrDefault().Fees = uow.Repository<Ex_CheckRequest_Fees>().GetData().Where(d => d.Ex_CheckRequest.ID == CheckRequest_ID_Ex).FirstOrDefault().Total_Amount;
                ...
                var Fees_Item_Shift = (from sh in entities.Ex_RequestCommittee_Shift
                                       ...
                                       where sh.Ex_RequestCommittee.Ex_CheckRequest.ID == CheckRequest_ID_Ex
                ...
                Fees_Dat.List_Fees_Martyrs = Fees_Martyrslist;
                #endregion
                ...
                var Fees_Sample = (from sm in entities.Ex_CheckRequest_SampleData
                                   ...
                                   where sm.Ex_RequestCommittee.Ex_CheckRequest.ID == CheckRequest_ID_Ex
                ...
                var Fees_Treatment = (from rq in entities.Ex_RequestCommittee
                                      ...
                                      where td.Ex_RequestCommittee.Ex_CheckRequest.ID == CheckRequest_ID_Ex
        ...
        public Dictionary<string, object> Get_FeesEx(long CheckRequest_ID_Ex, int FeesEx, List<string> Device_Info)
        {
            try

            {
                Dictionary<string, SqlDbType> paramters_Type = new Dictionary<string, SqlDbType>();
                paramters_Type.Add("Ex_CheckRequest", SqlDbType.BigInt);

                Dictionary<string, string> paramters_Data = new Dictionary<string, string>();
                paramters_Data.Add("Ex_CheckRequest", CheckRequest_ID_Ex.ToString());


                var data = uow.Repository<FeesExDTO>().CallStored("FeesEx", paramters_Type,
                paramters_Data, Device_Info).Where(a=>a.Ex_ShiftID!=null&& a.Ex_EngID!=null).ToList();
                //data = data.Max();
                return uow.Repository<Object>().DataReturn((int)DTO.HelperClasses.Enums.Success.GetData, data);
            }
            catch (Exception ex)
        ...
        //noura
        public Dictionary<string, object> Get_FeesNew(string Ex_CheckRequest_Number, List<string> Device_Info)
        {
            try
            {
                PlantQuarantineEntities entities = new PlantQuarantineEntities();
