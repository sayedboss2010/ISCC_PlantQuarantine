...
{
    public class Ex_Committee_Quick_BLL
    {
        private UnitOfWork uow;



        public Ex_Committee_Quick_BLL()
        {
            uow = new UnitOfWork();
        }
        ...
        public Dictionary<string, object> GetEX_CheckRequestDetails(List<Ex_Committee_Quick_DTO> EX_CheckRequest_ID, List<string> Device_Info)
        {
            try
            {

                //long EX_CheckRequest_ID = _EX_CheckRequest_ID.FirstOrDefault().reqId;
                var ListEx = EX_CheckRequest_ID.Select(a => a.EX_CheckRequest_ID).ToList();
                PlantQuarantineEntities entities = new PlantQuarantineEntities();
                string lang = Device_Info[2];
                //var CheckRequestDetails = (from cc in entities.Ex_CheckRequest
                //                           where ListEx.Contains( cc.ID)//== EX_CheckRequest_ID
                //                           select new Ex_Committee_Quick_DTO
                //                           {
                //                               EX_CheckRequest_ID = cc.ID,
                //                               EX_CheckRequest_Number = cc.CheckRequest_Number,
                //                               OutLet_Name = cc.Outlet.Ar_Name,
                //                               OutLet_ID = cc.Outlet.ID,
                //                               EX_Items_Quick=(from i in entities.Ex_CheckRequest_Items
                //                                               where i.Ex_CheckRequest_ID== cc.ID
                //                                               select new EX_Items_Quick
                //                                               {
                //                                                   ID_EX_Items=i.ID,
                //                                                   EX_ItemsName=i.Item_ShortName.Item.Name_Ar,
                //                                                   EX_Item_ShortName = i.Item_ShortName.ShortName_Ar,
                //                                                   EX_Items_Lot_Quick= (from l in entities.Ex_CheckRequest_Items_Lot_Category
                //                                                                        where l.Ex_CheckRequest_Items_ID== i.ID
                //                                                                        select new EX_Items_Lot_Quick
                //                                                                        {
                //                                                                            ID_Lot = l.ID,
                ...
                var CheckRequestDetails = (from cc in entities.Ex_CheckRequest
                                           join i in entities.Ex_CheckRequest_Items on cc.ID equals i.Ex_CheckRequest_ID
                                           join l in entities.Ex_CheckRequest_Items_Lot_Category on i.ID equals l.Ex_CheckRequest_Items_ID
                                           where ListEx.Contains(cc.ID)//== EX_CheckRequest_ID
                                           select new Ex_Committee_Quick_DTO
                                           {
                                               EX_CheckRequest_ID = cc.ID,
                                               ...
                                               EX_ItemsName = i.Item_ShortName.Item.Name_Ar,
                                               EX_Item_ShortName = i.Item_ShortName.ShortName_Ar,
                                               EX_Item_ShortName_ID = i.Item_ShortName.ID,
                                               ID_Lot = l.ID,
                                               Lot_Number = l.Lot_Number,
                                           }).ToList();
                ...
                //                {
                //                    long? ID_Lot = item.ID_Lot;

                //                    EX_Lot_Result_Status Result_Status_Empty = new EX_Lot_Result_Status();

                //                    Result_Status_Empty.LotData_ID = ID_Lot;
                //                    Result_Status_Empty.commite_No = 0;
                //                    Result_Status_Empty.Status_Name = "اللوط مصاب";
                //                    Result_Status_Empty.Nots_Result_Status = "اللوط مصاب";
                //                    Result_Status_Empty.IS_Status = 0;
                //                    Result_Status_Empty.IS_Status_Committee = 0;
                //                    Result_Status_Empty.Is_Continue = 0;

                //                    List<EX_Lot_Result_Status> lst = new List<EX_Lot_Result_Status>();
                //                    lst.Add(Result_Status_Empty);
                //                    item.list_Lot_Result_Status = lst;
                //                }
                //                else
                //                {

                //                    Dictionary<string, SqlDbType> paramters_Type = new Dictionary<string, SqlDbType>();
                //                    paramters_Type.Add("Lot_ID", SqlDbType.BigInt);
                //                    Dictionary<string, string> paramters_Data = new Dictionary<string, string>();

                //                    paramters_Data.Add("Lot_ID", item.ID_Lot.ToString());
                //                    var Lot_Result_Status = uow.Repository<EX_Lot_Result_Status>().CallStored("Ex_CheckLot_Commity", paramters_Type,
                ...
                //                        Result_Status_Empty.LotData_ID = ID_Lot;
                //                        Result_Status_Empty.commite_No = 1;
                //                        Result_Status_Empty.Status_Name = "لجنة فحص-صادر	";
                //                        Result_Status_Empty.Nots_Result_Status = "لجنة فحص-صادر	";
                //                        Result_Status_Empty.IS_Status = 1;
                //                        Result_Status_Empty.IS_Status_Committee = 1;
                //                        Result_Status_Empty.Is_Continue = 1;

                //                        EX_Lot_Result_Status Result_Status_Empty_sample = new EX_Lot_Result_Status();

                //                        Result_Status_Empty_sample.LotData_ID = ID_Lot;
                //                        Result_Status_Empty_sample.commite_No = 3;
                //                        Result_Status_Empty_sample.Status_Name = "لجنة سحب	";
                //                        Result_Status_Empty_sample.Nots_Result_Status = "لجنة سحب	";
                //                        Result_Status_Empty_sample.IS_Status = 1;
                //                        Result_Status_Empty_sample.IS_Status_Committee = 1;
                //                        Result_Status_Empty_sample.Is_Continue = 1;

                //                        List<EX_Lot_Result_Status> lst = new List<EX_Lot_Result_Status>();
                //                        lst.Add(Result_Status_Empty);
                //                        lst.Add(Result_Status_Empty_sample);
                //                        item.list_Lot_Result_Status = lst;
                //                    }
                //                    else
                //                    {
                ...
                //                        // لو سحب
                //                        long? ID_Lot33 = item.ID_Lot;
                //                        EX_Lot_Result_Status Result_Status_Empty_sample = new EX_Lot_Result_Status();

                //                        Result_Status_Empty_sample.LotData_ID = ID_Lot33;
                //                        Result_Status_Empty_sample.commite_No = 3;
                //                        Result_Status_Empty_sample.Status_Name = "لجنة سحب	";
                //                        Result_Status_Empty_sample.Nots_Result_Status = "لجنة سحب	";
                //                        Result_Status_Empty_sample.IS_Status = 1;
                ...
                //                        List<EX_Lot_Result_Status> lst33 = new List<EX_Lot_Result_Status>();
                //                        lst33.Add(Result_Status_Empty_sample);
                //                        //
                //                        //حالة اللوط فى جشنى وتم انتهاء العمل

                //                        if (Lot_Result_Status.Where(a => a.commite_No == 2 && a.IS_Status == 11).Count() > 0)
                //                        {
                //                            if (Lot_Result_Status.Where(a => a.commite_No == 2 && a.IS_Status == 10).Count() == 0) // لا يوجد اعادة
                //                            {
                //                                var Lot_Result_Ex_CommitteeGashny = entities.Ex_CommitteeResult.Where(lr => lr.LotData_ID == item.ID_Lot
                //                                                                          && lr.Ex_RequestCommittee.CommitteeType_ID == 2
                //                                                                          && lr.CommitteeResultType_ID == 1).Select(lr => lr.LotData_ID).ToList();

                //                                if (Lot_Result_Ex_CommitteeGashny.Count() > 0)
                //                                {
                //                                    // جايب لجنة جشني شغاله وخلصانة
                //                                    long? ID_Lot = item.ID_Lot;

                //                                    EX_Lot_Result_Status Result_Status_Empty = new EX_Lot_Result_Status();

                //                                    Result_Status_Empty.LotData_ID = ID_Lot;
                //                                    Result_Status_Empty.commite_No = 2;
                //                                    Result_Status_Empty.Status_Name = "لجنة جشني	";
                //                                    Result_Status_Empty.Nots_Result_Status = "لجنة جشني		";
                //                                    Result_Status_Empty.IS_Status = 11;
                //                                    Result_Status_Empty.IS_Status_Committee = 0;
                //                                    Result_Status_Empty.Is_Continue = 0;
                ...
                //                                    item.list_Lot_Result_Status = lst.Union(Lot_Result_Status.Where(a => a.commite_No != 2 && a.IS_Status != 11).ToList()).ToList();
                //                                }
                //                                else
                //                                {
                ...
                //                            else
                //                            {
                //                                var x = Lot_Result_Status.Where(a => a.commite_No == 2 && a.IS_Status == 11).ToList();

                //                                var Lot_Result_Status2 = Lot_Result_Status.Except(x).ToList();
                //                                item.list_Lot_Result_Status = Lot_Result_Status2;
                //                            }
                //                        }
                //                        else
                //                        {
                ...
                //                        // else
                //                        //{
                //                        //    item.list_Lot_Result_Status = Lot_Result_Status.Where(a => a.commite_No != 2 && a.IS_Status != 11).ToList();
                //                        //}

                //                        item.list_Lot_Result_Status = item.list_Lot_Result_Status.Union(lst33).ToList();
        ...
        public Dictionary<string, object> Insert_Committee(EX_RequestCommittee_QuickDTO entity, List<string> Device_Info)
        {
            ...
            try
            {
                ...
                using (PlantQuarantineEntities context = new PlantQuarantineEntities())
                {
                    ...
                    {
                        foreach (var CheckRequestList in entity.ListEX_CheckRequest_Quick)
                        {
                            foreach (var CommitteeTypeList in entity.ListCommitteeType_Quick)
                            {
                                ...
                                if (CommitteeTypeList.CommitteeType_ID == 1 || CommitteeTypeList.CommitteeType_ID == 2)
                                {
                                    var CheckCommity_Check_Gashny = (from r in context.Ex_RequestCommittee
                                                                     join cr in context.Ex_CommitteeResult on r.ID equals cr.Committee_ID
                                                                     where r.ExCheckRequest_ID == CheckRequestList.EX_CheckRequest_ID
                                                                     && r.CommitteeType_ID == CommitteeTypeList.CommitteeType_ID
                                                                     && allLot.Contains(cr.LotData_ID)
                                                                    && (r.Delegation_Date >= DateTimeDate
                                                                    || (r.IsPaid == true && r.IsFinishedAll == true))
                                                                     select cr.LotData_ID).ToList();

                                    var exists_Check_Gashny = allLot.Except(CheckCommity_Check_Gashny).ToList();
                                    if (exists_Check_Gashny.Count() > 0)
                                    {

                                        #region Ex_RequestCommittee
                                        ...
                                        RequestCommittee.ID = Committe_ID;
                                        RequestCommittee.ExCheckRequest_ID = CheckRequestList.EX_CheckRequest_ID;
                                        RequestCommittee.CommitteeType_ID = CommitteeTypeList.CommitteeType_ID;
                                        RequestCommittee.ExCommitteeCheckLocation_ID = entity.EX_CommitteeCheckLocation_ID;
                                        RequestCommittee.Delegation_Date = CommitteeTypeList.Delegation_Date;
                                        if (CommitteeTypeList.CommitteeType_ID == 2 || CommitteeTypeList.CommitteeType_ID == 1 || CommitteeTypeList.CommitteeType_ID == 3)//فى حالة الجشني
                                        {
                                            RequestCommittee.IsApproved = true;
                                        }
                                        else
                                        {
                                            RequestCommittee.IsApproved = null;
                                        }
                                        RequestCommittee.Status = false;
                                        RequestCommittee.User_Creation_Id = entity.ListCommitteeType_Quick.FirstOrDefault().User_Creation_Id;
                                        RequestCommittee.User_Creation_Date = DateTime.Now;
                                        int ShiftCount = 0;
                                        if(entity.List_Committee_Shift != null)
                                        {
                                            ShiftCount = entity.List_Committee_Shift.Where(a => a.Committee_ID == CommitteeTypeList.CommitteeType_ID).Count();
                                        }
                                        
                                        int engCount = 0;
                                        if(entity.List_Committee_Eng != null)
                                        {
                                            engCount = entity.List_Committee_Eng.Where(a => a.Committee_ID == CommitteeTypeList.CommitteeType_ID).Count();
                                        }
                                        if (ShiftCount == 0&& engCount == 0&& CommitteeTypeList.CommitteeType_ID != 3 && CommitteeTypeList.CommitteeType_ID != 6)
                                        {
                                            RequestCommittee.IsPaid = true;
                                        }
                                        ...
                                        {
                                            foreach (var com_empList in entity.com_emp)
                                            {
                                                long _Employee_Id = long.Parse(com_empList.Employee_Id.ToString());

                                                var Comm_Employee = new CommitteeEmployee
                                                {
                                                    Committee_ID = Committe_ID,
                                                    Employee_Id = _Employee_Id,
                                                    ISAdmin = com_empList.ISAdmin,
                                                    OperationType = operationType,
                                                    User_Creation_Id = entity.ListCommitteeType_Quick.FirstOrDefault().User_Creation_Id,
                                        ...
                                        if (entity.List_Committee_Shift != null)
                                        {
                                            ...
                                            {
                                                foreach (var Committee_ShiftList in entity.List_Committee_Shift)
                                                {
                                                    if (Committee_ShiftList.Committee_ID == CommitteeTypeList.CommitteeType_ID)
                                                    {
                                                        var id = uow.Repository<Object>().GetNextSequenceValue_Long("Ex_RequestCommittee_Shift_seq");
                                                        var _Amount = decimal.Parse(Committee_ShiftList.Amount.ToString());
                                                        var Committee_Shift = new Ex_RequestCommittee_Shift
                                                        {
                                                            ID = id,
                                                            Ex_RequestCommittee_ID = Committe_ID,
                                                            ShiftTiming_ID = Committee_ShiftList.ShiftTiming_ID,
                                                            Count = Committee_ShiftList.Count,
                                                            Amount = _Amount,
                                                            User_Creation_Id = entity.ListCommitteeType_Quick.FirstOrDefault().User_Creation_Id,
                                                            User_Creation_Date = DateTime.Now,
                                                        };
                                        ...
                                        if (entity.List_Committee_Eng != null)
                                        {
                                            if (entity.List_Committee_Eng.Count > 0)
                                            {
                                                foreach (var item_FeesENg in entity.List_Committee_Eng)
                                                {
                                                    if (item_FeesENg.Committee_ID == CommitteeTypeList.CommitteeType_ID)
                                                    {
                                                        var id_Eng = uow.Repository<Object>().GetNextSequenceValue_Long("Ex_RequestCommittee_Fees_ENG_seq");
                                                        var _Value = decimal.Parse(item_FeesENg.Value.ToString());
                                                        var Committee_Request_Fees_ENG = new Ex_RequestCommittee_Fees_ENG
                                                        {
                                                            ID = id_Eng,
                                                            Ex_RequestCommittee_ID = Committe_ID,
                                                            Ex_Fees_Type_ID = item_FeesENg.Ex_Fees_Type_ID,
                                                            //Count = item_Fees.Count,
                                                            Num_Eng = item_FeesENg.Num_Eng,
                                                            Value = _Value,
                                                            User_Creation_Id = entity.ListCommitteeType_Quick.FirstOrDefault().User_Creation_Id,
                                                            User_Creation_Date = DateTime.Now,
                                                        };
                                        ...
                                        #endregion
                                        foreach (var itemLot in entity.ListCommittee_Items_Lot.Where(a => a.EX_CheckRequest_ID == CheckRequestList.EX_CheckRequest_ID))
                                        {
                                            var CheckLot = CheckCommity_Check_Gashny.Where(a => a.Value == itemLot.LotData_ID).Count();
                                            #region الفحص
                                            if (CheckLot == 0)
                                            {
                                                ...
                                                var CommitteeResult = new Ex_CommitteeResult
                                                {
                                                    ...
                                                    Committee_ID = Committe_ID,
                                                    Ex_Request_Item_Id = itemLot.EX_Request_Item_Id,
                                                    LotData_ID = itemLot.LotData_ID,
                                                    EmployeeId = itemLot.EmployeeId,
                                                    //CommitteeResultType_ID = itemLot.CommitteeResultType_ID,
                                                    Date = itemLot.Date,
                                                    ...
                                                    //Weight = item.Weight,
                                                    //Notes = item.Notes,
                                                    IS_Total = itemLot.IS_Total,
                                                    Item_ShortName_ID = itemLot.Item_ShortName_ID,
                                                    User_Creation_Id = entity.ListCommitteeType_Quick.FirstOrDefault().User_Creation_Id,
                                                    User_Creation_Date = DateTime.Now,
                                        ...
                                        dto2.User_Creation_Id = entity.ListCommitteeType_Quick.FirstOrDefault().User_Creation_Id;
                                        ...
                                        #endregion
                                        entity.message += "تم الحفظ بنجاح" + CheckRequestList.EX_CheckRequest_ID + "";
                                    }
                                    else
                                    {
                                ...
                                if (CommitteeTypeList.CommitteeType_ID == 3)
                                {
                                    ...
                                    var CheckCommity_SampleData = (from r in context.Ex_RequestCommittee
                                                                   join sm in context.Ex_CheckRequest_SampleData on r.ID equals sm.Ex_RequestCommittee_ID
                                                                   where r.ExCheckRequest_ID == CheckRequestList.EX_CheckRequest_ID
                                                                   && r.CommitteeType_ID == CommitteeTypeList.CommitteeType_ID
                                                                   && allLot.Contains(sm.LotData_ID)
                                                                   && all_AnalysisLabType.Contains(sm.AnalysisLabType_ID)
                                                                  && (r.Delegation_Date >= DateTimeDate
                                                                  || (r.IsPaid == true && r.IsFinishedAll == true))
                                                                   select sm.LotData_ID).ToList();

                                    var exists_Commity_SampleData = allLot.Except(CheckCommity_SampleData).ToList();
                                    if (exists_Commity_SampleData.Count() > 0)
                                    {
                                        int Count_Samp = 1;
                                        #region Ex_RequestCommittee
                                        var operationType = 73; //ask
                                        ...
                                        foreach (var itemLot in entity.ListCommittee_Items_Lot.Where(a => a.EX_CheckRequest_ID == CheckRequestList.EX_CheckRequest_ID))
                                        {
                                            var CheckLot = CheckCommity_SampleData.Where(a => a.Value == itemLot.LotData_ID).Count();
                                            #region السحب
                                            ...
                                            if (CheckLot == 0)
                                            {
                                                ...
                                                foreach (var Item_Analysis in entity.List_SampleData)
                                                {
                                                    ...
                                                    CheckRequest_SampleData.ID = EX_CheckRequest_SampleData_ID;
                                                    CheckRequest_SampleData.AnalysisLabType_ID = Item_Analysis.AnalysisLabType_ID;
                                                    CheckRequest_SampleData.Ex_RequestCommittee_ID = Committe_ID;
                                                    CheckRequest_SampleData.Ex_Request_Item_Id = itemLot.EX_Request_Item_Id;
                                                    CheckRequest_SampleData.Item_ShortName_ID = itemLot.Item_ShortName_ID;
                                                    CheckRequest_SampleData.LotData_ID = itemLot.LotData_ID;
                                                    if (Count_Samp == 1)
                                                    {
                                                        CheckRequest_SampleData.Amount = Item_Analysis.Amount;
                                                        CheckRequest_SampleData.Count_Sample = Item_Analysis.Count_Sample;
                                                    }
                                                    CheckRequest_SampleData.User_Creation_Id = entity.ListCommitteeType_Quick.FirstOrDefault().User_Creation_Id;
                                                    CheckRequest_SampleData.User_Creation_Date = DateTime.Now;
                                                    CheckRequest_SampleData.IsPrint = Item_Analysis.IsPrint;
                                                    CheckRequest_SampleData.IS_Total = itemLot.IS_Total;

                                                    CheckRequest_SampleData.Sample_BarCode = Item_Analysis.Sample_BarCode;
                                ...
                                if (CommitteeTypeList.CommitteeType_ID == 6)
                                {
                                    ...
                                    if (exists_Commity_SampleData.Count() > 0)
                                    {
                                        ...
                                        RequestCommittee.Status = entity.Status;
                                        RequestCommittee.User_Creation_Id = entity.ListCommitteeType_Quick.FirstOrDefault().User_Creation_Id;
                                        RequestCommittee.User_Creation_Date = DateTime.Now;
                                        if (entity.List_Committee_Shift == null && entity.List_Committee_Eng == null
                                            && CommitteeTypeList.CommitteeType_ID != 3 && CommitteeTypeList.CommitteeType_ID != 6)
                                        {
                                            RequestCommittee.IsPaid = true;
                                        }
                                        context.Ex_RequestCommittee.Add(RequestCommittee);
                        ...
                        //   //     var Check_Check_ID = context.Ex_CommitteeResult
                        //   //.Where(a => a.LotData_ID == item.LotData_ID
                        //   //&& a.CommitteeResultType_ID == item.CommitteeResultType_ID
                        //   //&& a.Ex_RequestCommittee.Is_Cancel != 1
                        //   //&& a.Ex_RequestCommittee.IsFinishedAll != false

                        //   //).ToList();

                        //        var Check_AnalysisLabType_ID = context.Ex_CheckRequest_SampleData
                        //            .Where(a => a.LotData_ID == Item_Analysis.LotData_ID
                        //      && a.AnalysisLabType.AnalysisTypeID == AnalysisTypeID.AnalysisTypeID
                        //      &&a.Ex_RequestCommittee.IsApproved !=false
                        //        && a.Ex_RequestCommittee.Is_Cancel != 1
                        //   && a.Ex_RequestCommittee.IsFinishedAll != false
                        //      ).ToList();
                        ...
                        //        dto2.ID_Table_Action = 3;
                        //        dto2.ID_TableActionValue = Committe_ID;
                        //        dto2.User_Creation_Id = CommitteeTypeList.User_Creation_Id;
                        //        dto2.User_Creation_Date = DateTime.Now;
                        //        dto2.NOTS = " تم عمل لجنة علي الطلب ";
                        //        dto2.User_Type_ID = 127;
                        //        dto2.Type_log_ID = 133;
        ...
        public Dictionary<string, object> Get_Fees(string Ex_CheckRequest_Number, List<string> Device_Info)
        {
            try
            {
                ...
                var Fees_Sample = (from sm in entities.Ex_CheckRequest_SampleData
                                   ...
                                   //&& (sm.Ex_RequestCommittee.Delegation_Date >= DateTime.Now )
                                   group sm by new
                                   {
                                       ID = sm.ID,
        ...
        public Dictionary<string, object> GetEX_AllTreatmentDataForShortnameId(long EX_CheckRequest_ID, long ExportCountry_Id, long shortnameId, List<string> Device_Info)
        {
...

            {
                ...
                List<TreatmentMethodsDTO> treatmentMethodsList2 = new List<TreatmentMethodsDTO>();
                var lookup = entities.EX_Choose_Treatment.Where(a => a.Ex_CheckRequest_ID == EX_CheckRequest_ID
                && a.Item_ShortName_id == shortnameId)
