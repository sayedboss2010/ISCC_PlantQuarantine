...
{
    public class Im_Constrain_TypeBLL : IGenericBLL<Im_Constrain_TypeDTO>
    {
        private UnitOfWork uow;

        public Im_Constrain_TypeBLL()
        {
            uow = new UnitOfWork();
        }
        ...
        public Dictionary<string, object> Find(object Id, List<string> Device_Info)
        {
            try
            {
                Im_Constrain_Type ca = uow.Repository<Im_Constrain_Type>().Findobject(Id);
                var _DTO = Mapper.Map<Im_Constrain_Type, Im_Constrain_TypeDTO>(ca);
        ...
        {
            var count = uow.Repository<Im_Constrain_Type>().GetData().Where(p => p.User_Deletion_Id == null).Count();
        ...
        public Dictionary<string, object> GetAll(int pageSize, int index, List<string> Device_Info)
        {
            ...
            try
            {
                var data = new List<Im_Constrain_Type>();
                string lang = Device_Info[2];

                if (pageSize == -1 && index == -1)
                {
                    data = uow.Repository<Im_Constrain_Type>().GetData().Where(a => a.User_Deletion_Id == null).OrderBy(A => (lang == "1" ? A.Ar_Name : A.En_Name)).ToList();
                }
                else
                {
                    data = uow.Repository<Im_Constrain_Type>().GetData().Where(a => a.User_Deletion_Id == null).OrderBy(A => (lang == "1" ? A.Ar_Name : A.En_Name)).Skip(index).Take(pageSize).ToList();
                }

                var dataDTO = data.Select(Mapper.Map<Im_Constrain_Type, Im_Constrain_TypeDTO>);
        ...
        public Dictionary<string, object> GetAll(string arName, string enName, int pageSize, int index, string jtSorting, List<string> Device_Info)
        {
            try
            {
                ...
                {
                    data = data = uow.Repository<Im_Constrain_Type>().GetData().Where(a =>
                ...
                {
                    data = uow.Repository<Im_Constrain_Type>().GetData().Where(a => a.User_Deletion_Id == null).ToList();
                    data_Count = data.Count();
                }
                else
                {
                    data = uow.Repository<Im_Constrain_Type>().GetData().Where(a =>
                ...
                var dataDto = data.OrderBy(A => (lang == "1" ? A.Ar_Name : A.En_Name)).Skip(index).Take(pageSize)
                    .Select(x => new Im_Constrain_TypeDTO()
                    {
                        Ar_Name = x.Ar_Name,
                        En_Name = x.En_Name,
                       ...
                       // Descreption_En = x.Descreption_En,
                        ...
                        IsActive = x.IsActive,
                        //  Is_IPPC = x.Is_IPPC,
                        // ListUnions_Id = uow.Repository<Union_ConstrainType>().GetData().Where(u => u.ConstrainType_ID == x.ID && u.User_Deletion_Id == null).Select(u => u.Union_ID).ToList(),
                ...
                dic.Add("ConstrainType_Data", dataDto);
        ...
        public bool GetAny(Im_Constrain_TypeDTO entity)
        {
            //var obj = entity as Im_Constrain_TypeDTO;
            return uow.Repository<Im_Constrain_Type>().GetAny(p => (p.User_Deletion_Id == null &&
                                        (p.Ar_Name == entity.Ar_Name && p.En_Name == entity.En_Name)) && (entity.ID == 0 ? true : p.ID != entity.ID));
        }

        public Dictionary<string, object> Insert(Im_Constrain_TypeDTO entity, List<string> Device_Info)
        {
            try
            {
                ...
                if (!GetAny(entity))
                {
                    ...
                    var id = uow.Repository<Object>().GetNextSequenceValue_Byte("Im_Constrain_Type_seq");
                    //entity.ID =int.Parse( id.ToString());
                    entity.ID = id;
                    entity.IsActive = entity.IsActive;
                    var obj = entity as Im_Constrain_TypeDTO;
                    var CModel = Mapper.Map<Im_Constrain_Type>(obj);
                    var data = uow.Repository<Im_Constrain_Type>().InsertReturn(CModel);
                    uow.SaveChanges();
        ...
        public Dictionary<string, object> Update(Im_Constrain_TypeDTO entity, List<string> Device_Info)
        {
            try
            {
                entity.Ar_Name = entity.Ar_Name.Trim();
                ...
                {
                    var obj = entity as Im_Constrain_TypeDTO;
                    Im_Constrain_Type CModel = uow.Repository<Im_Constrain_Type>().Findobject(obj.ID);
                    ...
                    uow.Repository<Im_Constrain_Type>().Update(Co);
                    uow.SaveChanges();

                    var empDTO = Mapper.Map<Im_Constrain_Type, Im_Constrain_TypeDTO>(Co);
        ...
        public Dictionary<string, object> Delete(DeleteParameters dto, List<string> Device_Info)
        {
            try
            {
                var Cmodel = uow.Repository<Im_Constrain_Type>().Findobject(dto.id);
                if (Cmodel != null)
                {
                    Cmodel.User_Deletion_Date = dto._DateNow;
                    Cmodel.User_Deletion_Id = dto.Userid;
                    uow.Repository<Im_Constrain_Type>().Update(Cmodel);
                    uow.SaveChanges();
        ...
        //    int index, int ConstrainTypeList, List<string> Device_Info)
        //{
        ...
        //        var data = uow.Repository<Im_Constrain_Type>().GetData().Where(a => a.User_Deletion_Id == null
        //        && ((ConstrainTypeList == 0 ? 1 == 1 : a.ID == ConstrainTypeList))).ToList();

        //        var dataDto = data.OrderBy(A => A.ID).Skip(index).Take(pageSize).Select(Mapper.Map<Im_Constrain_Type, Im_Constrain_TypeDTO>);

        //        var data2 = uow.Repository<Union_ConstrainType>().GetData().Where(a => a.User_Deletion_Id == null
        //        ).ToList();

        //        var dataDto2 = data2.Select(Mapper.Map<Union_ConstrainType, Union_ConstrainTypeDTO>);


        //        List<CustomConstrainType_UnionList> CustList = new List<CustomConstrainType_UnionList>();

        //        foreach (var item in dataDto)
        //        {
        //            CustomConstrainType_UnionList cDto = new CustomConstrainType_UnionList();
        //            cDto.ID = item.ID;
        ...
        //                           join o in dataDto2
        //                           on c.ID equals o.ConstrainType_ID
        ...
        //        dic.Add("Count_Data", data_Count);
        //        dic.Add("ConstrainType_Data", CustList);
        //        return uow.Repository<Object>().DataReturn((int)DTO.HelperClasses.Enums.Success.Insert, dic);
        //    }
        //    catch (Exception ex)
        //    {
        ...
        {
            string lang = Device_Info[2];
            var data = uow.Repository<Im_Constrain_Type>().GetData().Where(a => a.User_Deletion_Id == null && a.IsActive == true).
                Select(c => new CustomOption
                {  //change display lang
                    DisplayText = (lang == "1" ? c.Ar_Name : c.En_Name),
                    Value = c.ID
