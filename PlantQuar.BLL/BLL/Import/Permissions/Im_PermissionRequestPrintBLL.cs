...
{
    public class Im_PermissionRequestPrintBLL
    {
        private UnitOfWork uow;

        public Im_PermissionRequestPrintBLL()
        {
            uow = new UnitOfWork();
        }
        public Dictionary<string, object> GetImPermissionPrintDetails
           (decimal? ImPermission_Number, short User_Creation_Id, List<string> Device_Info)
        {
            try
            {
                ...
                var itemss = uow.Repository<Im_PermissionItems>().GetData().Where(i => i.Im_PermissionRequest_ID == permissionPrintDetails.Im_PermissionRequest_ID).Select(v => new Itemss
                {
                    ...
                    // subPartName = uow.Repository<SubPart>().GetData().Where(s=>s.ID == v.SubPart_id).FirstOrDefault().Name_Ar,
                    // Item_Permission_Number = v.Item_Permission_Number,
                    //GrossWeight = v.GrossWeight
                }).Distinct().ToList();
