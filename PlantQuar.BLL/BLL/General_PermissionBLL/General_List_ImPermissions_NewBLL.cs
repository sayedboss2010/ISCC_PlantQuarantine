...
{
    public class General_List_ImPermissions_NewBLL
    {
        private UnitOfWork uow;
        //dbPrivilageEntities entities1 = new dbPrivilageEntities();

        public General_List_ImPermissions_NewBLL()
        {
            uow = new UnitOfWork();
        }
        public Dictionary<string, object> GetImPermissionsList(int radio_ID, long? Company_ID,
            long? Country_ID, long? ShortName_ID, int? Type_Item, int? Type_Company,
            string Im_PermissionRequest_Num,int? operationCode, List<string> Device_Info)
        {
            try
            {               
                ...
                paramters_Data.Add("OperationType", operationCode.ToString());
                ...
                var request = uow.Repository<ImPermissionsListDTO>().CallStored("Get_List_General_PermissionRequest", paramters_Type,
        ...
        public Dictionary<string, object> GetImPermissionsList_filter
           (int Isacceppted,decimal? ImPermission_Number, int operationCode, List<string> Device_Info)
        {
            try
            {
                PlantQuarantineEntities entities = new PlantQuarantineEntities();
                ...
                var permissions = (from cc in entities.Im_PermissionRequest
                                   ...
                                   join co in entities.Countries on rr.ExportCountry_Id equals co.ID
                                   from oo in oos.DefaultIfEmpty()
                                   where rr.Im_OperationType == operationCode
                                   select new ImPermissionsListDTO
                                   {
                                       Im_PermissionRequest_ID = cc.ID,
         ...
         public Dictionary<string, object> GetImPermissionsList_filter_ActivePrint
           (decimal? ImPermission_Number, int operationCode, List<string> Device_Info)
        {
            try
            {
                PlantQuarantineEntities entities = new PlantQuarantineEntities();
               ...
               //}).FirstOrDefault();
                ...
                foreach (var per in permissions)
                {
                    ...
                    if (initiator != null)
                    {
                        ...
                        if (initiatir.QualitativeGroup_Id != null)
                        ...
                        if (initiatir.Item_ShortName_ID != null)
                        {
                            var itemShortNameId = initiatir.Item_ShortName_ID;
        ...
        //public Dictionary<string, object> Update(ActivePrintDto dto, int operationCode, List<string> Device_Info)
        {
           PlantQuarantineEntities db = new PlantQuarantineEntities();
            ...
            // need to check which one will search

            try
            {
                 var f = db.Im_PermissionRequest.FirstOrDefault(x => x.ImPermission_Number ==   dto.Im_PermissionRequestID);
                if (f != null)
                {
