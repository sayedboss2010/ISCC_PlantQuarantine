...
namespace PlantQuar.BLL.BLL.Stations
{
    public class StationBLL : IGenericBLL<StationDTO>
    {
        ...
        public Dictionary<string, object> Find(object Id, List<string> Device_Info)
        {
            ...
            try
            {
                Station entity = uow.Repository<Station>().Findobject(Id);
                var _DTO = Mapper.Map<Station, StationDTO>(entity);
        ...
        {
            var count = uow.Repository<Station>().GetData().Where(p => p.User_Deletion_Id == null).Count();
        ...
        public Dictionary<string, object> GetAll(int pageSize, int index, List<string> Device_Info)
        {
            try
            {
                var data = uow.Repository<Station>().GetData().Where(a => a.User_Deletion_Id == null).OrderBy(A => A.ID).Skip(index).Take(pageSize).ToList();
                var dataDTO = data.Select(Mapper.Map<Station, StationDTO>);
        ...
        public Dictionary<string, object> GetAll(string arName, string enName, int pageSize, int index, string jtSorting, List<string> Device_Info)
        {
            ...
            {            Dictionary<string, object> dic = new Dictionary<string, object>();
                ...
                {
                    data = uow.Repository<Station>().GetData().Where(a => a.User_Deletion_Id == null &&
                ...
                {
                    data = uow.Repository<Station>().GetData().Where(a => a.User_Deletion_Id == null).ToList();
                    data_Count = data.Count();
                }
                else
                {
                ...
                var dataDTO = data.OrderBy(A => (lang == "1" ? A.Ar_Name : A.En_Name)).Skip(index).Take(pageSize).Select(Mapper.Map<Station, StationDTO>);
        ...
        {
            return uow.Repository<Station>().GetAny(p => p.User_Deletion_Id == null &&
        ...
        public Dictionary<string, object> Insert(StationDTO entity, List<string> Device_Info)
        {
            try
            {
                ...
                if (!GetAny(entity))
                {
                    var CModel = Mapper.Map<Station>(entity);
                    CModel.ID = uow.Repository<Object>().GetNextSequenceValue_Long("Station_seq");
                    var data = uow.Repository<Station>().InsertReturn(CModel);
                    uow.SaveChanges();
        ...
        public Dictionary<string, object> Update(StationDTO entity, List<string> Device_Info)
        {
            try
            {
                ...
                if (!GetAny(entity))
                {
                    Station CModel = uow.Repository<Station>().Findobject(entity.ID);
                    ...
                    uow.Repository<Station>().Update(Co);
                    uow.SaveChanges();

                    var empDTO = Mapper.Map<Station, StationDTO>(Co);
...

        {
            try
            {
                var Cmodel = uow.Repository<Station>().Findobject(obj.id);
                if (Cmodel != null)
                {
                    Cmodel.User_Deletion_Date = obj._DateNow;
                    Cmodel.User_Deletion_Id = obj.Userid;
                    uow.Repository<Station>().Update(Cmodel);
                    uow.SaveChanges();
        ...
        {
             string lang = Device_Info[2];
            var data = uow.Repository<Station>().GetData().Where(lab => lab.User_Deletion_Id == null && lab.IsActive == true)
                .Select(c => new CustomOption
                {  //change display lang
                    DisplayText = (lang == "1" ? c.Ar_Name : c.En_Name),
                    Value = (int)c.ID }).OrderBy(a => a.DisplayText).ToList();
        ...
        {
             string lang = Device_Info[2];
            var data = uow.Repository<Station>().GetData().Where(a => a.User_Deletion_Id == null).Select(c => new CustomOption
            {  //change display lang
                DisplayText = (lang == "1" ? c.Ar_Name : c.En_Name),
                Value = (int)c.ID }).OrderBy(a => a.DisplayText).ToList();
            //set default value fz 17-4-2019
        ...
        public Dictionary<string, object> GetAccridatedStations(List<string> Device_Info)
        {
            try
            {
                var data = from s in uow.Repository<Station>().GetData()
