    ...
    public class StationPaymentBLL
    {
        ...
        public Dictionary<string, object> Find(long Id, List<string> Device_Info)
        {
            try
            {
                ...
                List<FixedFeesAmountDTO> fees = (from cc in entities.Station_Accreditation_Committee_FixedFeesAmount
                                                 join FF in entities.FeesAmount_Fixed on cc.FixedFeesAmount_ID equals FF.ID
                                                 where cc.Station_Committee_Id == Id
                                                 select new FixedFeesAmountDTO
                                                 {
                                                     Amount = FF.Amount,
                                                     feesName = (lang == "1") ? FF.Name_Ar : FF.Name_En
                                                 }).ToList();



                //uow.Repository<Station_Accreditation_Committee_FixedFeesAmount>().GetData()
                ...
                dic.Add("stationCode", stationCode);
