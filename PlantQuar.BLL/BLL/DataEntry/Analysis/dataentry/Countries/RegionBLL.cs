...
{
   public class RegionBLL : IGenericBLL<RegionDTO>
    {
        ...
        public Dictionary<string, object> Find(object Id, List<string> Device_Info)
        {
            try
            {
                Region ca = uow.Repository<Region>().Findobject(Id);
                var _DTO = Mapper.Map<Region, RegionDTO>(ca);
        ...
        {
            var count = uow.Repository<Region>().GetData().Where(p => p.User_Deletion_Id == null).Count();
        ...
        public Dictionary<string, object> GetAll(int pageSize, int index, List<string> Device_Info)
        {
            ...
            try
            {
                ...
                {
                    data = uow.Repository<Region>().GetData().Where(a => a.User_Deletion_Id == null).OrderBy(A => (lang == "1" ? A.Name_Ar : A.Name_En)).ToList();
                }
                else
                {
                    data = uow.Repository<Region>().GetData().Where(a => a.User_Deletion_Id == null).OrderBy(A => (lang == "1" ? A.Name_Ar : A.Name_En)).Skip(index).Take(pageSize).ToList();
                }

                var dataDTO = data.Select(Mapper.Map<Region, RegionDTO>);
            ...
            //{
            //    string lang = Device_Info[2];
            //    var data = uow.Repository<Region>().GetData().Where(a => a.User_Deletion_Id == null).OrderBy(A => (lang == "1" ? A.Name_Ar : A.Name_En)).ToList();

            //   // var data = uow.Repository<Region>().GetData().Where(a => a.User_Deletion_Id == null).OrderBy(A => (lang == "1" ? A.Name_Ar : A.Name_En)).Skip(index).Take(pageSize).ToList();
            //    var dataDTO = data.Select(x => new RegionDTO()
            //    {
            //        Name_Ar = x.Name_Ar,
            //        Name_En = x.Name_En,
            //        ID = x.ID,
            //        IsActive = x.IsActive,
            //        Is_IPPC = x.Is_IPPC,
            //        //ListUnions_Id = uow.Repository<Union_Region>().GetData().Where(u => u.Region_ID == x.ID && u.User_Deletion_Id == null).Select(u => u.Union_ID).ToList(),
        ...
        public Dictionary<string, object> GetAll(string arName, string enName, int pageSize, int index, string jtSorting, List<string> Device_Info)
        {
            try
            {
                ...
                {
                    data = data = uow.Repository<Region>().GetData().Where(a =>
                ...
                {
                    data = uow.Repository<Region>().GetData().Where(a =>
                ...
                var dataDto = data.OrderBy(A => (lang == "1" ? A.Name_Ar : A.Name_En)).Skip(index).Take(pageSize)
                    .Select(x => new RegionDTO()
                    {
                        ...
                        IsActive = x.IsActive,
                        //  Is_IPPC = x.Is_IPPC,
                        Country_ID =x.Country_ID,// uow.Repository<Country>().GetData().Where(u => u.ID == x.Country_ID && u.User_Deletion_Id == null).Select(u => u.ID).ToList(),
                        User_Deletion_Id = x.User_Deletion_Id,
                        User_Creation_Date = x.User_Creation_Date,
                        User_Creation_Id = x.User_Creation_Id,
                        User_Deletion_Date = x.User_Deletion_Date,
        ...
        public bool GetAny(RegionDTO entity)
        {
            //var obj = entity as RegionDTO;
            return uow.Repository<Region>().GetAny(p => (p.User_Deletion_Id == null &&
                                        (p.Name_Ar == entity.Name_Ar && p.Name_En == entity.Name_En)) && (entity.ID == 0 ? true : p.ID != entity.ID));
        }

        public Dictionary<string, object> Insert(RegionDTO entity, List<string> Device_Info)
        {
            try
            {
                ...
                if (!GetAny(entity))
                {
                    ...
                    var id = uow.Repository<Object>().GetNextSequenceValue_Long("Regions_seq");
                    //entity.ID =int.Parse( id.ToString());
                    entity.ID = id;

                    var obj = entity as RegionDTO;
                    var CModel = Mapper.Map<Region>(obj);
                    var data = uow.Repository<Region>().InsertReturn(CModel);
                    uow.SaveChanges();
        ...
        public Dictionary<string, object> Update(RegionDTO entity, List<string> Device_Info)
        {
            try
            {
                entity.Name_Ar = entity.Name_Ar.Trim();
                ...
                {
                    var obj = entity as RegionDTO;
                    Region CModel = uow.Repository<Region>().Findobject(obj.ID);
                    ...
                    uow.Repository<Region>().Update(Co);
                    uow.SaveChanges();

                    var empDTO = Mapper.Map<Region, RegionDTO>(Co);
        ...
        public Dictionary<string, object> Delete(DeleteParameters dto, List<string> Device_Info)
        {
            try
            {
                var Cmodel = uow.Repository<Region>().Findobject(dto.id);
                if (Cmodel != null)
                {
                    Cmodel.User_Deletion_Date = dto._DateNow;
                    Cmodel.User_Deletion_Id = dto.Userid;
                    uow.Repository<Region>().Update(Cmodel);
                    uow.SaveChanges();
        ...
        //    int index, int RegionList, List<string> Device_Info)
        //{
        ...
        //        var data = uow.Repository<Region>().GetData().Where(a => a.User_Deletion_Id == null
        //        && ((RegionList == 0 ? 1 == 1 : a.ID == RegionList))).ToList();

        //        var dataDto = data.OrderBy(A => A.ID).Skip(index).Take(pageSize).Select(Mapper.Map<Region, RegionDTO>);

        //        var data2 = uow.Repository<Union_Region>().GetData().Where(a => a.User_Deletion_Id == null
        //        ).ToList();

        //        var dataDto2 = data2.Select(Mapper.Map<Union_Region, Union_RegionDTO>);


        //        List<CustomRegion_UnionList> CustList = new List<CustomRegion_UnionList>();

        //        foreach (var item in dataDto)
        //        {
        //            CustomRegion_UnionList cDto = new CustomRegion_UnionList();
        //            cDto.ID = item.ID;
        ...
        public Dictionary<string, object> FillDrop_Add(List<string> Device_Info, short Country_ID)
        {

            string lang = Device_Info[2];
            var data = uow.Repository<Region>().GetData()
        ...
        public Dictionary<string, object> FillDrop_Edit(List<string> Device_Info, long Country_ID)
        {
            //string lang = Device_Info[2];
            //var data = uow.Repository<Region>().GetData()
            //    .Where(c => c.Country_ID == Country_ID && c.User_Deletion_Id == null && c.IsActive == true)
            //    .Select(c => new CustomOption
            //    { //change display lang
            //        DisplayText = (lang == "1" ? c.Name_Ar : c.Name_En),
            //        Value = long.Parse(c.ID.ToString())
            ...
            string lang = Device_Info[2];
            var data = uow.Repository<Region>().GetData()
                .Where(a => a.User_Deletion_Id == null && a.IsActive == true && a.Country_ID == Country_ID)
                .Select(c => new CustomOptionLongId
                {  //change display lang
                    DisplayText = (lang == "1" ? c.Name_Ar : c.Name_En),
                    Value = c.ID
