...
{
    public class ShippingAgencyBLL
    {
        private UnitOfWork uow;

        public ShippingAgencyBLL()
        {
            uow = new UnitOfWork();
        }
        ...
        public Dictionary<string, object> Find(object Id, List<string> Device_Info)
        {
            try
            {
                ShippingAgency entity = uow.Repository<ShippingAgency>().Findobject(Id);
                var empDTO = Mapper.Map<ShippingAgency, ShippingAgencyDTO>(entity);
        ...
        public Dictionary<string, object> GetAll(int pageSize, int index, List<string> Device_Info)
        {
            ...
            try
            {
                var data = new List<ShippingAgency>();
                string lang = Device_Info[2];

                if (pageSize == -1 && index == -1)
                {
                    data = uow.Repository<ShippingAgency>().GetData().Where(a => a.User_Deletion_Id == null).OrderBy(A => (lang == "1" ? A.Name_Ar : A.Name_En)).ToList();
                }
                else
                {
                    data = uow.Repository<ShippingAgency>().GetData().Where(a => a.User_Deletion_Id == null).OrderBy(A => (lang == "1" ? A.Name_Ar : A.Name_En)).Skip(index).Take(pageSize).ToList();
                }

                var dataDTO = data.Select(Mapper.Map<ShippingAgency, ShippingAgencyDTO>);
        ...
        public Dictionary<string, object> GetAll(string arName, string enName, int pageSize, int index, string jtSorting, List<string> Device_Info)
        {
            try
            {
                ...
                {
                    data = uow.Repository<ShippingAgency>().GetData().Where(a => a.User_Deletion_Id == null &&
                ...
                dic.Add("ShippingAgency_Data", data);
        ...
        public bool GetAny(ShippingAgencyDTO entity)
        {
            var obj = entity as ShippingAgencyDTO;
            return uow.Repository<ShippingAgency>().GetAny(p => p.User_Deletion_Id == null && (obj.ID == 0 ? true : p.ID != obj.ID) &&
                                        (p.Name_Ar == obj.Name_Ar));
        }

        public Dictionary<string, object> Insert(ShippingAgencyDTO entity, List<string> Device_Info)
        {
            try
            {
                ...
                if (!GetAny(entity))
                {
                    //ShippingAgencies_seq
                       var id = uow.Repository<Object>().GetNextSequenceValue_Long("ShippingAgencies_SEQ");
                    entity.ID = id;
                    var CModel = Mapper.Map<ShippingAgency>(entity);
               
                    uow.Repository<ShippingAgency>().InsertRecord(CModel);
                    uow.SaveChanges();
        ...
        public Dictionary<string, object> Update(ShippingAgencyDTO entity, List<string> Device_Info)
        {
            try
            {
                entity.Name_Ar = entity.Name_Ar.Trim();
                ...
                {
                    var obj = entity as ShippingAgencyDTO;
                    ShippingAgency CModel = uow.Repository<ShippingAgency>().Findobject(obj.ID);
                    ...
                    uow.Repository<ShippingAgency>().Update(Co);
                    uow.SaveChanges();

                    var empDTO = Mapper.Map<ShippingAgency, ShippingAgencyDTO>(Co);
        ...
        public Dictionary<string, object> Delete(DeleteParameters dto, List<string> Device_Info)
        {
            try
            {
                var Cmodel = uow.Repository<ShippingAgency>().Findobject(dto.id);
                if (Cmodel != null)
                {
                    Cmodel.User_Deletion_Date = dto._DateNow;
                    Cmodel.User_Deletion_Id = dto.Userid;
                    uow.Repository<ShippingAgency>().Update(Cmodel);
                    uow.SaveChanges();
